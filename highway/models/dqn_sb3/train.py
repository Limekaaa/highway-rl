...
try:
    # Prefer package import when run as a module: python -m highway.models.dqn_sb3.train
...
try:
    ...
    # When running as a script (python highway/models/dqn_sb3/train.py), the repo root
...
def make_seeded_env(seed: int, render_mode=None):
    env = gym.make(SHARED_CORE_ENV_ID, config=SHARED_CORE_CONFIG, render_mode=render_mode)
    env.reset(seed=seed)
    try:
...
def train(
    ...
    log_dir = config.get("log_dir", "logs/dqn_sb3")
    ...
    output_formats = ["stdout", "csv"]
    if config.get("agent_config", {}).get("tensorboard_log"):
        output_formats.append("tensorboard")
    new_logger = configure(str(log_dir), output_formats)

    train_env = make_seeded_env(seed=seed, render_mode=None)
    ...
    if eval_freq and eval_episodes:
        eval_env = make_seeded_env(seed=seed + 1, render_mode=None)
    ...
    agent_config = dict(config.get("agent_config", {}))
    ...
    agent_dqn.set_logger(new_logger)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a DQN agent (Stable-Baselines3).")
    parser.add_argument(
        "--config",
        type=str,
        default="config.json",
